...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"sb";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "sb";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "Fake Plugin to recieve sb plugincmds.";
}
		...
		case CMD_SEND:  /* /test command <command> */
			if(param1) {
				...
				if(pluginID) {
					...
					ts3Functions.sendPluginCommand(serverConnectionHandlerID, pluginID, param1, PluginCommandTarget_SERVER, NULL, NULL);
				} else {
...
void ts3plugin_onPluginCommandEvent(uint64 serverConnectionHandlerID, const char* pluginName, const char* pluginCommand) {
	...
	//"name=sb data=ping:23221"
	int position = pluginCommandString.find(":", 0);
	std::string sender = pluginCommandString.substr(position+1, std::string::npos);
